...
public:
  ...
  bool SelectIntrinsic(SDNode *Node);
...
bool EVMDAGToDAGISel::SelectIntrinsic(SDNode *Node) {

  unsigned IID = cast<ConstantSDNode>(Node->getOperand(1))->getZExtValue();
  switch (IID) {
  case Intrinsic::evm_printf: {
      const SDValue &Chain = Node->getOperand(0);
      ...
      Ops.push_back(Chain);
      MachineSDNode *call =
          CurDAG->getMachineNode(EVM::PRINTF_r, SDLoc(Node), Node->getVTList(), Ops);
      ReplaceNode(Node, call);
      return true;
  }
  }
...
void EVMDAGToDAGISel::Select(SDNode *Node) {
  ...
  switch (Opcode) {
    ...
    case ISD::INTRINSIC_VOID:
      if (SelectIntrinsic(Node)) return;
      break;
  }
