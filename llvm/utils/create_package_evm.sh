...
cmake $SOURCE_DIR -G Ninja -DCMAKE_BUILD_TYPE=Release -DLLVM_ENABLE_PROJECTS="clang" -DLLVM_TARGETS_TO_BUILD="EVM" -DLLVM_EXPERIMENTAL_TARGETS_TO_BUILD=EVM -DEVM_HOST=ON -DLLVM_CCACHE_BUILD=ON -DLLVM_ENABLE_LLD=ON -DCMAKE_INSTALL_PREFIX=./install -DLLVM_INSTALL_TOOLCHAIN_ONLY=ON -DDEFAULT_SYSROOT=/usr
